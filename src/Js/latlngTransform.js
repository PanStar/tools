...
function Coordtransform(){
    var that = this;
    //定义一些常量
    that.x_PI = 3.14159265358979324 * 3000.0 / 180.0;
    that.PI = 3.1415926535897932384626;
    that.a = 6378245.0;
    that.ee = 0.00669342162296594323;
    /**
    ...
    this.gcj02tobd09 = function(lng, lat){
        var z = Math.sqrt(lng * lng + lat * lat) + 0.00002 * Math.sin(lat * that.x_PI);
        var theta = Math.atan2(lat, lng) + 0.000003 * Math.cos(lng * that.x_PI);
    ...
    this.wgs84togcj02 = function(lng, lat) {
        if (that.out_of_china(lng, lat)) {
            ...
            var radlat = lat / 180.0 * that.PI;
            var magic = Math.sin(radlat);
            magic = 1 - that.ee * magic * magic;
            var sqrtmagic = Math.sqrt(magic);
            dlat = (dlat * 180.0) / ((that.a * (1 - that.ee)) / (magic * sqrtmagic) * that.PI);
            dlng = (dlng * 180.0) / (that.a / sqrtmagic * Math.cos(radlat) * that.PI);
            var mglat = lat + dlat;
            var mglng = lng + dlng;
            return [mglng, mglat];
        }
    ...
    this.wgs84tobd09 = function(lng, lat) {
        var lnglat = that.wgs84togcj02(parseFloat(lng),parseFloat(lat));
        return that.gcj02tobd09(lnglat[0],lnglat[1]);
    }
    /**
    ...
    this.bd09towgs84 = function(lng, lat) {
        var lnglat = that.bd09togcj02(parseFloat(lng),parseFloat(lat));
        return that.gcj02towgs84(lnglat[0],lnglat[1]);
    }
}
